    ...
    <div className="min-h-screen bg-white font-sans">
      ...
      <div className="bg-primary py-24 text-white text-center">
        <div className="container mx-auto px-4">
          <h1 className="text-5xl font-heading font-black tracking-tighter mb-4 text-accent-gold leading-none">The Price of Perfection</h1>
          <p className="text-white/70 font-bold text-[10px]">Nigerian Standards • Transparent Rates • ₦</p>
        </div>
      </div>
      ...
      <section className="py-24 container mx-auto px-4 max-w-6xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-0 mb-24">
          {[
            ...
            <div key={i} className={cn(
              ...
              <h3 className={cn("text-xs font-black mb-6", plan.popular ? "text-accent-gold" : "text-gray-400")}>{plan.tier}</h3>
              <p className="text-[11px] font-bold mb-10 opacity-70 leading-relaxed">{plan.desc}</p>
        ...
        <div className="space-y-16">
          {categories.map((cat) => (
            <div key={cat.name}>
              <h2 className="text-2xl font-black text-primary tracking-tighter mb-8 border-b-4 border-accent-gold inline-block">{cat.name}</h2>
              <div className="overflow-x-auto shadow-2xl">
                <table className="w-full text-left bg-white border-collapse">
                  <thead>
                    <tr className="bg-secondary/50 border-b-2 border-gray-100">
                      <th className="p-6 text-[10px] font-black text-primary ">Service Item</th>
                      <th className="p-6 text-[10px] font-black text-gray-400 ">Basic</th>
                      <th className="p-6 text-[10px] font-black text-accent-gold ">Premium</th>
                      <th className="p-6 text-[10px] font-black text-primary text-right">Elite</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-50">
                    {cat.items.map((item, idx) => (
                      <tr key={idx} className="hover:bg-secondary/20 transition-colors">
                        <td className="p-6 text-sm font-bold text-secondary-dark tracking-tight">{item.service}</td>
        ...
        <div className="mt-32 bg-secondary p-12 rounded-none border-t-8 border-primary">
          <div className="flex flex-col lg:flex-row items-center justify-between gap-12">
            <div className="max-w-md">
              <h2 className="text-3xl font-black text-primary tracking-tighter mb-4">Subscription Rewards</h2>
              <p className="text-gray-500 font-bold text-xs leading-relaxed">Automate your shine and save significantly with our recurring maintenance plans.</p>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6 w-full lg:w-auto">
              {discounts.map((d) => (
                <div key={d.frequency} className="bg-white p-6 text-center shadow-lg border-b-4 border-accent-gold">
                   <p className="text-[9px] font-black text-gray-400 mb-2">{d.frequency}</p>
      ...
      <section className="bg-secondary py-20">
        <div className="container mx-auto px-4 max-w-4xl text-center">
          <h2 className="text-3xl font-black text-primary tracking-tighter mb-8">Need a Custom Commercial Quote?</h2>
          <p className="text-gray-500 font-bold text-xs mb-10 leading-relaxed">For office buildings, plazas, and large estates, we provide tailored pricing based on square footage and frequency.</p>
          <Link to="/contact">
            <Button className="bg-primary text-white font-black px-12 py-5 rounded-none shadow-xl">Contact Sales</Button>
          </Link>
        </div>
      </section>
    </div>
