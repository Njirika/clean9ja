...
export function CustomerDashboard() {
  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen bg-secondary/30 flex flex-col items-center justify-center p-6 text-center">
        <h1 className="text-3xl font-black text-primary tracking-tighter mb-3">Please log in</h1>
        <p className="text-gray-500 font-bold text-[11px] mb-8">Sign in to view your dashboard.</p>
        <Link to="/auth">
          <Button className="bg-primary text-white font-black px-10 py-4 rounded-none">Login / Sign up</Button>
        </Link>
      </div>
    );
  }
  ...
  const Empty = ({ label }: { label: string }) => (
    <div className="py-16 text-center text-gray-400 font-black text-[11px]">{label}</div>
  );
    ...
    <div className="min-h-screen bg-secondary/30 pb-24 lg:pb-12 font-sans flex flex-col">
      ...
      <header className="sticky top-0 z-40 bg-primary text-white py-4 px-6 flex justify-between items-center shadow-lg lg:hidden">
        ...
        <div className="flex items-center space-x-3 bg-[#144718] px-3 py-1.5 rounded-full border border-white/10">
          ...
          <span className="text-[9px] font-black text-accent-gold">{currentUser?.first_name}</span>
        </div>
      </header>
        ...
        <div className="container mx-auto px-6 relative z-10 flex justify-between items-center">
          <div>
            <div className="flex items-center space-x-2 bg-[#144718] px-3 py-1 rounded-full border border-white/10 w-fit mb-4">
              ...
              <span className="text-[9px] font-black text-accent-gold">{currentUser?.role || 'Member'}</span>
            </div>
            <h1 className="text-4xl font-heading font-black tracking-tighter">My Account</h1>
            <p className="text-white/60 font-bold text-[10px] mt-1">Welcome back, {displayName}</p>
          </div>
          <Link to="/book">
            <Button className="bg-accent-orange text-white font-black text-[10px] px-6 py-4 rounded-none shadow-xl hover:bg-white hover:text-accent-orange transition-all">
      ...
      <div className="container mx-auto px-4 lg:px-6 flex-grow mt-6 lg:-mt-16 z-10">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <main className="lg:col-span-3 pb-16">
            {error && (
              <div className="mb-6 p-4 bg-red-50 border-l-4 border-red-500 text-[10px] font-black text-red-600 tracking-wider">{error}</div>
            )}
              ...
              <div className="space-y-8 animate-in fade-in duration-300">
                <div className="bg-gradient-to-r from-primary via-primary/95 to-primary-bright text-white p-8 rounded-3xl shadow-2xl relative overflow-hidden border-b-8 border-accent-gold">
                  ...
                  <div className="relative z-10 max-w-lg space-y-4">
                    <div>
                      <p className="text-[10px] font-black text-accent-gold">Welcome back</p>
                      <h2 className="text-3xl lg:text-4xl font-heading font-black tracking-tighter leading-none mt-1">{displayName}</h2>
                    </div>
                    <p className="text-xs text-white/75 font-semibold tracking-wider leading-relaxed">Ready to schedule your next spotless shine? Book a vetted field specialist today.</p>
                    <Link to="/book">
                      <Button className="bg-white text-primary hover:bg-accent-orange hover:text-white font-black text-[10px] px-6 py-4 rounded-full shadow-lg transition-all duration-300">
                  ...
                  {[
                    ...
                    <button key={i} onClick={() => setActiveTab(metric.tab as Tab)} className="p-5 lg:p-6 bg-white border border-gray-100 rounded-3xl shadow-md text-left hover:shadow-lg hover:border-accent-gold transition-all duration-300 flex flex-col justify-between h-40 group">
                      <div className="flex justify-between items-start w-full">
                        <span className="text-[8.5px] font-black text-gray-400 leading-none">{metric.label}</span>
                        <div className={`p-2.5 rounded-full shrink-0 ${metric.bg} group-hover:scale-110 transition-transform`}>
                          <metric.icon className="w-4 h-4" />
                        </div>
                      </div>
                      <div className="space-y-1">
                        <p className="text-2xl font-black text-primary tracking-tighter leading-none">{metric.value}</p>
                        <p className="text-[8px] font-black text-gray-400 leading-none">{metric.sub}</p>
                      </div>
                    </button>
                  ))}
                </div>
            ...
            {activeTab === 'bookings' && (
              <div className="bg-white p-6 lg:p-8 border border-gray-100 shadow-xl space-y-6 animate-in fade-in duration-300 rounded-none">
                <div className="border-b border-gray-100 pb-4 flex justify-between items-center">
                  <div>
                    <h2 className="text-2xl font-black text-primary tracking-tighter">My Bookings</h2>
                    <p className="text-[9px] text-gray-400 font-bold ">Active dispatch runs & cleaning history</p>
                  </div>
                  <Link to="/book"><Button size="sm" className="bg-accent-orange text-white font-black text-[9px] px-4 py-2">New Booking</Button></Link>
                </div>
                    ...
                    {bookings.map((b) => (
                      <div key={b.id} className="p-6 border border-gray-100 bg-white hover:border-primary transition-all flex flex-col md:flex-row justify-between items-start md:items-center gap-6 shadow-sm border-l-4 border-l-accent-gold">
                        <div className="space-y-2">
                          <div className="flex items-center space-x-3">
                            <h4 className="font-black text-primary text-sm tracking-tight">{b.service?.name || 'Cleaning service'}</h4>
                            <span className="px-2.5 py-0.5 text-[8px] font-black rounded-full bg-accent-gold/15 text-primary">{b.status}</span>
                          </div>
                          <p className="text-[10px] font-bold text-gray-400 flex items-center"><Calendar className="w-3.5 h-3.5 mr-2 text-primary shrink-0" />{fmtDate(b.scheduledDate)} {b.scheduledTimeSlot ? `• ${b.scheduledTimeSlot}` : ''}</p>
                          {b.cleaner?.user && (
                            <p className="text-[10px] font-bold text-gray-500 flex items-center bg-secondary/50 px-3 py-1 w-fit"><User className="w-3.5 h-3.5 mr-2 text-primary shrink-0" />Assigned: {b.cleaner.user.firstName} {b.cleaner.user.lastName}</p>
                          )}
                        </div>
                        <div className="text-right w-full md:w-auto border-t md:border-t-0 pt-4 md:pt-0 flex md:flex-col justify-between items-center md:items-end">
                          <p className="text-[9px] font-bold text-gray-400 md:mb-1">Ref: {b.bookingReference}</p>
            ...
            {activeTab === 'subscriptions' && (
              <div className="bg-white p-6 lg:p-8 border border-gray-100 shadow-xl space-y-6 animate-in fade-in duration-300 rounded-none">
                <div className="border-b border-gray-100 pb-4">
                  <h2 className="text-2xl font-black text-primary tracking-tighter">Hygiene Subscriptions</h2>
                  <p className="text-[9px] text-gray-400 font-bold ">Recurring weekly/monthly washes</p>
                </div>
                      ...
                      <div key={s.id} className="p-6 border border-gray-100 bg-white shadow-sm border-t-4 border-t-primary flex flex-col justify-between h-52">
                        <div>
                          <div className="flex justify-between items-start mb-4">
                            <h4 className="font-black text-primary text-sm tracking-tight">{s.service?.name || 'Plan'}</h4>
                            <span className={`px-2 py-0.5 text-[8px] font-black rounded-full ${s.status === 'active' ? 'bg-primary-bright/10 text-primary-bright' : 'bg-gray-100 text-gray-400'}`}>{s.status}</span>
                          </div>
                          <p className="text-[10px] font-black text-accent-gold mb-1">{naira(s.pricePerVisit)} / visit</p>
                          <p className="text-[10px] font-bold text-gray-400 tracking-wider">Frequency: {s.frequency}</p>
                        </div>
                        <div className="pt-4 border-t border-gray-50">
                          <p className="text-[8px] font-bold text-gray-400 ">Next Run</p>
                          <p className="text-[10px] font-black text-primary ">{fmtDate(s.nextBookingDate)}</p>
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            {activeTab === 'payments' && (
              <div className="bg-white p-6 lg:p-8 border border-gray-100 shadow-xl space-y-6 animate-in fade-in duration-300 rounded-none">
                <div className="border-b border-gray-100 pb-4">
                  <h2 className="text-2xl font-black text-primary tracking-tighter">Payment History</h2>
                  <p className="text-[9px] text-gray-400 font-bold ">Invoices & transactions</p>
                </div>
                {bookings == null ? <Empty label="Loading…" /> : bookings.filter((b) => b.paymentStatus === 'paid').length === 0 ? (
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-left">
                      <thead className="bg-secondary border-b border-gray-100"><tr>{['Reference', 'Date', 'Service', 'Amount', 'Status'].map((h) => <th key={h} className="px-6 py-4 text-[9px] font-black text-primary ">{h}</th>)}</tr></thead>
                      <tbody className="divide-y divide-gray-50 bg-white">
                        {bookings.filter((b) => b.paymentStatus === 'paid').map((b) => (
                          <tr key={b.id} className="hover:bg-gray-50 transition-colors">
                            <td className="px-6 py-4 text-[10px] font-black text-primary ">{b.bookingReference}</td>
                            <td className="px-6 py-4 text-xs font-bold text-gray-500">{fmtDate(b.scheduledDate)}</td>
                            <td className="px-6 py-4 text-xs font-bold text-gray-600 ">{b.service?.name}</td>
                            <td className="px-6 py-4 text-xs font-black text-primary">{naira(b.finalPrice ?? b.quotedPrice)}</td>
                            <td className="px-6 py-4 text-[9px] font-black text-primary-bright ">{b.paymentStatus}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
            ...
            {activeTab === 'addresses' && (
              <div className="bg-white p-6 lg:p-8 border border-gray-100 shadow-xl space-y-6 animate-in fade-in duration-300 rounded-none">
                <div className="border-b border-gray-100 pb-4 flex justify-between items-center">
                  <div>
                    <h2 className="text-2xl font-black text-primary tracking-tighter">Saved Addresses</h2>
                    <p className="text-[9px] text-gray-400 font-bold ">Cleaning coordinates for quick checkout</p>
                  </div>
                  <Button onClick={() => setShowAddAddr(!showAddAddr)} className="bg-primary text-white font-black text-[9px] px-4 py-2.5 rounded-none flex items-center"><Plus className="w-3.5 h-3.5 mr-1" /> Add Address</Button>
                </div>
                ...
                {showAddAddr && (
                  <form onSubmit={handleAddAddress} className="p-6 border border-accent-gold bg-accent-gold/5 space-y-4 animate-in slide-in-from-top-4 duration-300">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-[9px] font-black text-primary mb-1">Label</label>
                      ...
                      <div><label className="block text-[9px] font-black text-primary mb-1">City</label><input required value={newCity} onChange={(e) => setNewCity(e.target.value)} placeholder="e.g. Lekki" className="w-full h-10 border border-gray-300 px-3 font-bold text-xs bg-white focus:outline-primary" /></div>
                      <div><label className="block text-[9px] font-black text-primary mb-1">State</label><input required value={newState} onChange={(e) => setNewState(e.target.value)} placeholder="e.g. Lagos" className="w-full h-10 border border-gray-300 px-3 font-bold text-xs bg-white focus:outline-primary" /></div>
                      <div><label className="block text-[9px] font-black text-primary mb-1">LGA</label><input value={newLga} onChange={(e) => setNewLga(e.target.value)} placeholder="e.g. Eti-Osa" className="w-full h-10 border border-gray-300 px-3 font-bold text-xs bg-white focus:outline-primary" /></div>
                      <div className="md:col-span-2"><label className="block text-[9px] font-black text-primary mb-1">Street Address</label><input required value={newStreet} onChange={(e) => setNewStreet(e.target.value)} placeholder="Building, street, area" className="w-full h-10 border border-gray-300 px-3 font-bold text-xs bg-white focus:outline-primary" /></div>
                    </div>
                    <div className="flex space-x-3 pt-2">
                      <Button type="submit" disabled={savingAddr} className="bg-primary text-white font-black text-[9px] px-6 py-3 rounded-none shadow-md disabled:opacity-50">{savingAddr ? 'Saving…' : 'Save Address'}</Button>
                      <button type="button" onClick={() => setShowAddAddr(false)} className="px-6 py-3 border border-gray-200 text-gray-400 hover:text-primary font-black text-[9px] bg-white">Cancel</button>
                    </div>
                  </form>
                )}
                      ...
                      <div key={a.id} className="p-6 border border-gray-100 bg-white hover:border-primary transition-all flex justify-between items-center shadow-sm border-l-4 border-l-primary">
                        <div>
                          <div className="flex items-center space-x-3 mb-2">
                            <h4 className="font-black text-primary text-xs tracking-wider">{a.label}</h4>
                            {a.isDefault && <span className="bg-primary/5 text-primary text-[8px] font-black px-2 py-0.5 rounded-full">Default</span>}
                          </div>
            ...
            {activeTab === 'settings' && (
              <div className="bg-white p-6 lg:p-8 border border-gray-100 shadow-xl space-y-8 animate-in fade-in duration-300 rounded-none">
                <div className="border-b border-gray-100 pb-4">
                  <h2 className="text-2xl font-black text-primary tracking-tighter">Account Settings</h2>
                  <p className="text-[9px] text-gray-400 font-bold ">Update your profile information</p>
                </div>
                <form onSubmit={handleSaveProfile} className="space-y-6">
                  {isSaved && <div className="p-4 bg-primary/10 border-l-4 border-primary text-[10px] font-black text-primary tracking-wider flex items-center"><ShieldCheck className="w-5 h-5 text-primary mr-2" /> Profile saved successfully!</div>}
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <label className="block text-[9px] font-black text-primary mb-2">Full Name</label>
                    ...
                    <div>
                      <label className="block text-[9px] font-black text-primary mb-2">Email Address</label>
                    ...
                    <div>
                      <label className="block text-[9px] font-black text-primary mb-2">Phone Number</label>
                  ...
                  <Button type="submit" disabled={savingProfile} className="bg-primary text-white font-black text-[10px] px-8 py-4 rounded-none shadow-md hover:bg-accent-orange transition-all disabled:opacity-50">{savingProfile ? 'Saving…' : 'Save Profile Settings'}</Button>
                </form>
              </div>
            )}
          </main>
          ...
          if (item.isCTA) {
            return (
              <Link key={item.id} to="/book" className="flex flex-col items-center justify-center flex-1 transition-all scale-110 -mt-5">
                ...
                <span className="text-[7.5px] font-black text-accent-gold mt-1">{item.label}</span>
              </Link>
            );
          }
          return (
            <button key={item.id} onClick={() => setActiveTab(item.id as Tab)} className={`flex flex-col items-center justify-center flex-1 transition-all ${isActive ? 'text-accent-gold scale-110 font-bold' : 'text-white/40 hover:text-white/80'}`}>
              ...
              <span className="text-[7.5px] font-black text-center truncate w-full leading-none">{item.label}</span>
            </button>
          );
        })}
      </nav>
