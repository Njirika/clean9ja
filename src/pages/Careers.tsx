    ...
    <div className="min-h-screen bg-white pb-20">
      ...
      <div className="bg-secondary py-24 border-b border-gray-100">
        <div className="container mx-auto px-4 text-center">
          <h1 className="text-5xl font-heading font-black text-primary tracking-tighter mb-4">Join The Pro Team</h1>
          <p className="text-gray-500 font-bold text-xs">Help Us Make Nigeria Spotless, One Space At A Time</p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 mb-20">
          {[
            ...
            <div key={i} className="flex flex-col items-center text-center p-8 bg-secondary/50 rounded-none border-b-4 border-accent-gold">
              ...
              <h3 className="text-xl font-black text-primary tracking-tighter mb-2">{v.title}</h3>
              <p className="text-sm text-gray-500 font-medium">{v.desc}</p>
            </div>
          ))}
        </div>

        <h2 className="text-3xl font-heading font-black text-primary tracking-tighter mb-12 border-b-8 border-accent-gold inline-block">Open Positions</h2>
            ...
            <Card key={i} className="p-10 rounded-none border-l-8 border-primary hover:border-accent-orange transition-all shadow-sm hover:shadow-2xl">
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8">
                <div className="flex-1">
                  <h3 className="text-2xl font-black text-primary tracking-tighter mb-4">{job.title}</h3>
                  <div className="flex flex-wrap gap-6 mb-6">
                    <div className="flex items-center text-xs font-black text-gray-400 "><Briefcase className="w-4 h-4 mr-2 text-accent-gold" /> {job.type}</div>
                    <div className="flex items-center text-xs font-black text-gray-400 "><MapPin className="w-4 h-4 mr-2 text-accent-gold" /> {job.location}</div>
                    <div className="flex items-center text-xs font-black text-gray-400 "><DollarSign className="w-4 h-4 mr-2 text-accent-gold" /> {job.salary}</div>
                  </div>
                  <p className="text-gray-500 font-medium leading-relaxed">{job.desc}</p>
                </div>
                <Button className="bg-primary text-white font-black px-10 py-5 rounded-none shadow-xl hover:bg-accent-orange">Apply Now</Button>
              </div>
            </Card>
          ))}
        </div>
