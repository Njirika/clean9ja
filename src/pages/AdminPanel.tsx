    ...
    <div className="min-h-screen bg-secondary/30 pb-24 lg:pb-12 font-sans flex flex-col">
      ...
      <header className="sticky top-0 z-40 bg-primary text-white py-4 px-6 flex justify-between items-center shadow-lg lg:hidden">
        ...
        <div className="flex items-center space-x-3 bg-[#144718] px-3 py-1.5 rounded-full border border-white/10">
          ...
          <span className="text-[9px] font-black text-accent-gold">SuperAdmin</span>
        </div>
      </header>
        ...
        <div className="container mx-auto px-6 relative z-10 flex justify-between items-center">
          <div>
            <div className="flex items-center space-x-2 bg-[#144718] px-3 py-1 rounded-full border border-white/10 w-fit mb-4">
              ...
              <span className="text-[9px] font-black text-accent-gold">Clean9ja Command Center</span>
            </div>
            <h1 className="text-4xl font-heading font-black tracking-tighter">HQ Operations</h1>
            <p className="text-white/60 font-bold text-[10px] mt-1">Lekki Dispatch Control & Administration</p>
          </div>
          <div className="flex space-x-4 bg-white/5 p-4 border border-white/10 rounded-none">
            <div className="text-center px-4 border-r border-white/10">
              <p className="text-[9px] font-black text-white/50 ">Active Hubs</p>
              <p className="text-xl font-black text-accent-gold">4 STATIONS</p>
            </div>
            <div className="text-center px-4">
              <p className="text-[9px] font-black text-white/50 ">NIN Vetting</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <aside className="lg:col-span-1 space-y-3 hidden lg:block">
            ...
            <div className="p-6 bg-white border border-gray-100 rounded-none border-t-4 border-t-accent-gold shadow-md space-y-4">
              <h4 className="font-black text-primary text-[10px] tracking-wider border-b border-gray-50 pb-2">Vetting Services</h4>
              <div className="flex items-center space-x-3 p-3 bg-primary/5 border-l-4 border-primary">
                <ShieldCheck className="w-5 h-5 text-primary shrink-0" />
                <p className="text-[8.5px] font-black text-primary leading-normal">
          ...
          <main className="lg:col-span-3 pb-16">
                  ...
                  {stats.map(s => (
                    <Card key={s.label} className="p-6 rounded-none border-b-4 border-accent-gold shadow-md bg-white">
                      <div className="flex justify-between items-start">
                        <div>
                          <p className="text-[8px] font-black text-gray-400 mb-1.5">{s.label}</p>
                          <p className="text-xl font-black text-primary tracking-tighter">{s.value}</p>
                        </div>
                ...
                <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
                  ...
                  <div className="xl:col-span-2">
                    <Card className="rounded-none border-t-8 border-primary shadow-xl overflow-hidden bg-white">
                      <div className="p-6 border-b border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-4">
                        <div>
                          <h3 className="font-black text-primary tracking-tighter text-lg">Mission Dispatch Control</h3>
                          <p className="text-[8.5px] text-gray-400 font-bold ">Assign staff cleaners to scheduled bookings</p>
                        </div>
                        <div className="flex items-center space-x-2 w-full sm:w-auto">
                          <div className="relative flex-grow sm:flex-none">
                            <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-300" />
                            <input className="w-full sm:w-40 pl-8 pr-3 py-1.5 border border-gray-100 font-bold text-[9px] focus:outline-primary bg-white" placeholder="Search..." />
                          </div>
                          ...
                          <thead className="bg-secondary/50 border-b border-gray-100">
                            <tr>
                              {['Ref', 'Customer', 'Service & Area', 'Status', 'Assign Specialist'].map(h => (
                                <th key={h} className="px-6 py-4 text-[9px] font-black text-primary ">{h}</th>
                              ))}
                            </tr>
                          </thead>
                          <tbody className="divide-y divide-gray-50 bg-white">
                            {(bookings || []).map(b => (
                              <tr key={b.id} className="hover:bg-gray-50 transition-colors">
                                <td className="px-6 py-5 text-[9px] font-black text-primary ">{b.bookingReference}</td>
                                <td className="px-6 py-5">
                                  <p className="text-xs font-bold text-gray-600">{b.cleaner?.user?.firstName || 'Customer'}</p>
                                  <p className="text-[8px] text-gray-400 font-bold ">{b.scheduledDate ? new Date(b.scheduledDate).toLocaleDateString() : '—'}</p>
                                </td>
                                <td className="px-6 py-5">
                                  <p className="text-[9px] font-black text-accent-gold leading-tight">{b.service?.name || 'Cleaning'}</p>
                                  <p className="text-[9px] font-bold text-gray-400">{b.address?.city || '—'}</p>
                                </td>
                                <td className="px-6 py-5">
                                  <span className={`px-2.5 py-0.5 text-[8px] font-black rounded-full ${getStatusColor(b.status as BookingStatus)}`}>
                                ...
                                <td className="px-6 py-5">
                                  <select 
                                    defaultValue=""
                                    className="text-[9px] font-black border border-gray-200 p-1.5 bg-white focus:outline-primary w-full max-w-[140px]"
                                  >
                  ...
                  <div className="xl:col-span-1">
                    <Card className="p-6 rounded-none border-t-8 border-primary shadow-xl bg-white space-y-6">
                      <div>
                        <h3 className="font-black text-primary tracking-tighter text-lg">Live Feeds</h3>
                        <p className="text-[8.5px] text-gray-400 font-bold ">Real-time dispatcher logs</p>
                      </div>
                      <div className="space-y-4">
                        {[1, 2, 3].map(i => (
                          <div key={i} className="flex items-center justify-between border-b border-gray-50 pb-3 last:border-0 last:pb-0">
                            <div>
                              <p className="font-black text-primary text-[10px]">Mission #{2840 + i}</p>
                              <p className="text-[9px] text-gray-400 font-bold ">
                                {i === 1 ? "Lekki • Emeka" : i === 2 ? "Maitama • Pending" : "PH • Specialist Funmi"}
                              </p>
                            </div>
                            <div className={`px-2 py-0.5 text-[7.5px] font-black ${i === 2 ? "bg-accent-orange/10 text-accent-orange" : "bg-primary/5 text-primary"}`}>
                              {i === 2 ? "Alert" : "Active"}
                            </div>
                          </div>
                        ))}
              ...
              <div className="space-y-8 animate-in fade-in duration-300">
                <Card className="p-6 lg:p-8 rounded-none border-t-8 border-accent-gold shadow-xl bg-white space-y-6">
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center border-b border-gray-100 pb-6 gap-4">
                    <div>
                      <h3 className="font-black text-primary tracking-tighter text-xl">Services & Pricing Manager</h3>
                      <p className="text-[9px] text-gray-400 font-bold mt-1">Configure service pricing tiers, base units, and visibility status</p>
                    </div>
                    <Button onClick={loadServices} disabled={loadingServices} className="bg-primary text-white font-black text-[9px] px-4 py-2.5 rounded-none flex items-center">
                      {loadingServices ? 'Refreshing...' : 'Refresh Services'}
                    </Button>
                  </div>

                  {serviceError && (
                    <div className="bg-red-50 text-red-600 px-4 py-3 text-xs font-bold border-l-4 border-red-500 mb-6 tracking-wider">
                      {serviceError}
                    </div>
                  )}
                  ...
                  {loadingServices && !services ? (
                    <div className="text-center py-12">
                      <p className="text-xs font-black text-gray-400 animate-pulse">Loading Clean9ja catalog...</p>
                    </div>
                  ) : services && services.length === 0 ? (
                    <div className="text-center py-12 bg-secondary/10 border border-dashed border-gray-200">
                      <p className="text-xs font-black text-gray-400 ">No services found in database catalog.</p>
                    </div>
                  ) : (
                            ...
                            <div className="flex items-start gap-4">
                              ...
                              <div>
                                <div className="flex items-center gap-2">
                                  <h4 className="font-black text-primary text-sm">{service.name}</h4>
                                  <span className={`text-[8px] font-black px-2 py-0.5 ${service.isActive ? 'bg-primary/10 text-primary' : 'bg-gray-100 text-gray-400'}`}>
                                    {service.isActive ? 'Active' : 'Inactive'}
                                  </span>
                                </div>
                                <p className="text-xs font-bold text-gray-400 text-[9px] mt-1">Category: {service.category}</p>
                              ...
                              {isEditing ? (
                                <div className="flex items-center gap-3 w-full sm:w-auto">
                                  <div className="w-full sm:w-32">
                                    <label className="block text-[8px] font-black text-primary mb-1">Base Price (₦)</label>
                                    <input 
                                      type="number"
                                      value={editPrice}
                                  ...
                                  <div className="w-full sm:w-32">
                                    <label className="block text-[8px] font-black text-primary mb-1">Price Unit</label>
                                    <select
                                      value={editUnit}
                                ...
                                <div className="text-left md:text-right">
                                  <p className="text-[8px] font-black text-gray-400 mb-1">Current Tier</p>
                                ...
                                {isEditing ? (
                                  <>
                                    <button 
                                      ...
                                      className="flex-grow sm:flex-none bg-primary text-white text-[9.5px] font-black px-4 py-3 hover:bg-accent-orange transition-all disabled:opacity-50"
                                    >
                                      {savingServiceId === service.id ? 'Saving...' : 'Save'}
                                    </button>
                                    <button 
                                      onClick={handleCancelEdit} 
                                      className="flex-grow sm:flex-none bg-gray-100 hover:bg-gray-200 text-gray-600 text-[9.5px] font-black px-4 py-3 transition-all"
                                    >
                                      Cancel
                                    </button>
                                  </>
                                  ...
                                  <button 
                                    ...
                                    className="w-full sm:w-auto bg-primary text-white text-[9.5px] font-black px-5 py-3 hover:bg-accent-orange transition-all shadow-md"
                                  >
                                    Edit Pricing
                                  </button>
                                )}
            ...
            {activeTab === 'recruitment' && (
              <div className="space-y-8 animate-in fade-in duration-300">
                <Card className="p-6 lg:p-8 rounded-none border-t-8 border-accent-gold shadow-xl bg-white space-y-6">
                  <div>
                    <h3 className="font-black text-primary tracking-tighter text-xl">Recruitment & Vetting</h3>
                    <p className="text-[9px] text-gray-400 font-bold ">Review employee submissions and NIN registrations</p>
                  </div>
                  ...
                  {applicants.length > 0 ? (
                      ...
                      {applicants.map(app => (
                        <div key={app.id} className="p-6 border border-gray-100 bg-white hover:border-accent-gold transition-all flex flex-col md:flex-row justify-between items-start md:items-center gap-6 shadow-sm border-l-4 border-l-primary">
                          <div className="space-y-2">
                            <div className="flex items-center space-x-3">
                              <h4 className="font-black text-primary text-sm">{app.name}</h4>
                              <span className="bg-accent-gold/15 text-primary text-[8px] font-black px-2 py-0.5">{app.experience}</span>
                            </div>
                            <p className="text-[10px] font-bold text-gray-400 ">Phone: {app.phone} • Hub Target: {app.location}</p>
                            <div className="inline-flex items-center space-x-2 text-[9px] font-black text-primary tracking-wider bg-secondary px-3 py-1">
                          ...
                          <div className="flex space-x-3 w-full md:w-auto">
                            <button 
                              ...
                              className="flex-grow md:flex-none bg-primary text-white text-[9.5px] font-black px-5 py-3 hover:bg-accent-orange transition-all"
                            >
                              Approve & Hire
                            </button>
                            <button 
                    ...
                    <div className="p-12 border-2 border-dashed border-gray-100 rounded-none text-center bg-secondary/10">
                      ...
                      <p className="text-xs text-gray-400 font-bold ">No pending applications to review.</p>
                    </div>
                  )}
                </Card>
              </div>
            ...
            {activeTab === 'settings' && (
              <div className="bg-white p-6 lg:p-8 border border-gray-100 shadow-xl animate-in fade-in duration-300 rounded-none space-y-8">
                <div className="border-b border-gray-100 pb-4">
                  <h2 className="text-2xl font-black text-primary tracking-tighter">System Settings</h2>
                  <p className="text-[9px] text-gray-400 font-bold ">Global configurations for background APIs & dispatch rules</p>
                </div>

                <form onSubmit={handleSaveSettings} className="space-y-6">
                  {settingsSaved && (
                    <div className="p-4 bg-primary/10 border-l-4 border-primary text-[10px] font-black text-primary tracking-wider animate-pulse flex items-center">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <label className="block text-[9px] font-black text-primary mb-2">Vetting Check Mode</label>
                      <select 
                        value={vettingMode}
                    ...
                    <div>
                      <label className="block text-[9px] font-black text-primary mb-2">SMS Integration Gateway</label>
                      <select 
                        value={smsGateway}
                    ...
                    <div>
                      <label className="block text-[9px] font-black text-primary mb-2">Gateway Bearer API Token</label>
                      <input 
                        type="text" 
                        value={apiKey}
                    ...
                    <div>
                      <label className="block text-[9px] font-black text-primary mb-2">Dispatch Buffer Window (Hours)</label>
                      <input 
                        type="number" 
                  ...
                  <div className="p-4 bg-accent-gold/10 border-2 border-accent-gold text-left flex items-start space-x-3">
                    ...
                    <div>
                      <p className="text-[9px] font-black text-primary mb-1">HQ Sandbox Rules</p>
                      <label className="flex items-center space-x-2 cursor-pointer mt-1">
                        ...
                        />
                        <span className="text-[9px] font-bold text-gray-500 ">Toggle development testing credentials bypass</span>
                      </label>
                    </div>
                  </div>

                  <Button type="submit" className="bg-primary text-white font-black text-[10px] px-8 py-4 rounded-none shadow-md hover:bg-accent-orange transition-all">
