    ...
    <section className="py-32 bg-white font-sans border-t border-gray-100 shadow-inner">
      <div className="container mx-auto px-4">
        <div className="text-center mb-20">
          <div className="inline-flex items-center text-accent-gold font-black text-[10px] mb-4">Knowledge is Power</div>
          <h2 className="text-5xl md:text-6xl font-heading font-black text-primary tracking-tighter mb-6">Clean9ja Blog</h2>
          <div className="w-24 h-2 bg-accent-gold mx-auto mb-8"></div>
          <p className="text-gray-400 font-bold text-xs">Expert advice for the Nigerian environment</p>
        </div>
          ...
          {blogs.slice(0, 3).map((post) => (
            <Card key={post.id} className="rounded-none border-t-8 border-accent-gold shadow-xl group cursor-pointer hover:-translate-y-2 transition-all duration-300">
              ...
              <div className="p-10 bg-white">
                ...
                <h3 className="text-xl font-black text-primary tracking-tighter mb-4 group-hover:text-accent-orange transition-colors line-clamp-2 leading-tight">
                  {post.title}
                </h3>
                <p className="text-gray-500 text-sm font-medium mb-10 leading-relaxed line-clamp-3">
                  {post.excerpt}
                </p>
                <Link to="/blog" className="inline-flex items-center text-accent-orange font-black text-[10px] hover:text-primary transition-colors group">
        ...
        <div className="mt-20 text-center">
          <Link to="/blog">
            <Button className="bg-primary text-white font-black px-12 py-5 rounded-none shadow-2xl hover:bg-accent-gold hover:text-primary transition-all">
  ...
  return (
    <div className="p-8 bg-white border-4 border-primary shadow-2xl">
      <div className="flex justify-between items-center mb-8 border-b-4 border-accent-gold pb-6">
        <h2 className="text-2xl font-black text-primary tracking-tighter flex items-center">
          <LayoutDashboard className="mr-3 w-6 h-6 text-accent-gold" /> Blog Command Center
        </h2>
        <Button onClick={() => setShowForm(!showForm)} className="bg-primary text-white font-black text-[10px] px-6 py-3 rounded-none">
        ...
        <form onSubmit={addPost} className="mb-12 p-8 bg-secondary/30 space-y-6 border-l-8 border-accent-gold animate-in fade-in slide-in-from-top-4 duration-300">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-4">
              <label className="block text-[10px] font-black text-primary ">Article Title</label>
            ...
            <div className="space-y-4">
              <label className="block text-[10px] font-black text-primary ">Category</label>
          ...
          <div className="space-y-4">
            <label className="block text-[10px] font-black text-primary ">Excerpt</label>
          ...
          {publishError && <div className="p-3 bg-red-50 border-l-4 border-red-500 text-[10px] font-black text-red-600 tracking-wider">{publishError}</div>}
          <Button type="submit" disabled={publishing} className="w-full bg-accent-orange text-white font-black py-5 shadow-2xl rounded-none transition-all hover:bg-primary disabled:opacity-60">
      ...
      <div className="space-y-4">
        <h3 className="text-xs font-black text-gray-400 mb-4">Published Articles</h3>
        {loading ? (
          <div className="py-8 text-center text-gray-400 font-black text-xs">Loading articles…</div>
        ) : blogs.length === 0 ? (
          <div className="py-8 text-center text-gray-400 font-black text-xs">No articles yet. Create your first post above.</div>
        ) : blogs.map(post => (
          <div key={post.id} className="flex items-center justify-between p-6 border border-gray-100 bg-white hover:bg-secondary/20 transition-colors">
            <div className="flex items-center space-x-6">
              ...
              <div>
                <h4 className="font-black text-primary text-sm">{post.title}</h4>
                <p className="text-[10px] font-bold text-gray-400 ">{post.date} • {post.category}</p>
              </div>
            </div>
